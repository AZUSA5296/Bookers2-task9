class SearchController < ApplicationController
  ...
  def match(model, value)                         #def search_forでhowが完全一致だった場合の処理
    if model == 'user'                            #modelがuserの場合の処理
      User.where(name: value)                     #whereでvalueと完全一致するnameを探します
    elsif model == 'book'                         #modelがbookの場合の処理
      User.where(name: value)                     #whereでvalueと完全一致するtitleを探します
      Book.where(title: value)
    end
  end

  def foward(model, value)                      #def search_forでhowが前方一致だった場合の処理
    if model == 'user'
  ...
  def backward(model, value)                      #def search_forでhowが後方一致だった場合の処理
    if model == 'user'
      User.where("name LIKE ?", "%#{value}")
    elsif model == 'book'
      Book.where("title LIKE ?", "%#{value}")
  ...
  def partical(model, value)                       #def search_forでhowが部分一致だった場合の処理
    if model == 'user'
      User.where("name LIKE ?", "%#{value}%")
    elsif model == 'book'
      Book.where("title LIKE ?", "%#{value}%")
  ...
  def search_for(how, model, value)       #searchアクションで定義した情報が引数に入っている
    ...
    when 'foward'                         #例えばhowがmatchの場合は def match の処理に進む
